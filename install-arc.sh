...
createconf() {
	...
	printf "%s\n" "rpcuser=$rpcuser" "rpcpassword=$rpcpass" "rpcallowip=127.0.0.1" "listen=1" "server=1" "daemon=1" "maxconnections=256" "externalip=$mnip" "bind=$mnip" "goldminenode=1" "goldminenodeprivkey=$MNPRIVKEY" "masternodeaddr=$mnip:7209" > $CONFDIR/$coin.conf
