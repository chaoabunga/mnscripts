...
createconf() {
	...
	printf "%s\n" "rpcuser=$rpcuser" "rpcpassword=$rpcpass" "rpcallowip=127.0.0.1" "listen=1" "server=1" "daemon=1" "maxconnections=256" "rpcport=4854" "externalip=$mnip" "bind=$mnip" "eternitynode=1" "eternitynodeprivkey=$MNPRIVKEY" "eternitynodeaddr=$mnip:4855" > $CONFDIR/$coin.conf
