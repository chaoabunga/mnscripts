#!/bin/sh
#Version 0.1.1.2
#Info: Installs PieXd daemon, Masternode based on privkey, and a simple web monitor.
...
noflags() {
    ...
    echo "Example: install-piex 8828uLaPreoz6dWNYS5wP7s8CwVfvQAeGUBo6kz7cwSSrZfHVFm"
    echo "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄"
    exit 1
}
...
success() {
	chaincoind
	message "SUCCESS! Your piexd has started."
	exit 0
}
...
compile() {
	cd PieXcoin #TODO: squash relative path
	...
	if [ $? -ne 0 ]; then error; fi
	message "Building PieX...this may take a few minutes..."
	sudo make
	if [ $? -ne 0 ]; then error; fi
	message "Installing PieX..."
	sudo make install
	if [ $? -ne 0 ]; then error; fi
}
...
createconf() {
	...
	printf "%s\n" "rpcuser=$rpcuser" "rpcpassword=$rpcpass" "rpcallowip=127.0.0.1" "listen=1" "server=1" "daemon=1" "maxconnections=256" "externalip=$mnip" "bind=$mnip" "masternode=1" "masternodeprivkey=$MNPRIVKEY" "masternodeaddr=$mnip:53472" > $CONFDIR/piex.conf
