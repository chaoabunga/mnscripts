...
noflags() {
	echo "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄"
    echo "Usage: install-arc [options]"
    echo "Valid options are:"
    echo "MASTERNODE_PRIVKEY(Required)"
    echo "Example: install-arc 6uQoYPjwCs6yknDS9g88XCUnVzcYF6EwXFwZC2JaidKB3x5PAFX"
    echo "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄"
    exit 1
}
...
success() {
	#variable later
	arcticcoind
	message "SUCCESS! Your arcticcoind has started."
	exit 0
}
...
compile() {
	cd arcticcoin #TODO: squash relative path
...
createconf() {
	...
	printf "%s\n" "rpcuser=$rpcuser" "rpcpassword=$rpcpass" "rpcallowip=127.0.0.1" "listen=1" "server=1" "daemon=1" "maxconnections=256" "rpcport=4854" "externalip=$mnip" "bind=$mnip" "goldminenode=1" "goldminenodeprivkey=$MNPRIVKEY" "masternodeaddr=$mnip:4855" > $CONFDIR/$coin.conf
...
createhttp() {
	...
	(crontab -l 2>/dev/null; echo "* * * * * echo MN Count:  > ~/web/stats.txt; /usr/local/bin/arcticcoin-cli masternode count >> ~/web/stats.txt; /usr/local/bin/arcticcoin-cli getinfo >> ~/web/stats.txt") | crontab -
	mnip=$(curl -s https://api.ipify.org)
